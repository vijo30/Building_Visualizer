...
def readOFF(filename, color):
    ...
    with open(filename, 'r') as file:
        ...
        color = np.tile(color, (num_vertices, 1))

        vertex_data = np.concatenate((vertices, color), axis=1)
        vertex_data = np.concatenate((vertex_data, normals), axis=1)
        ...
        for face in faces:
            vertex = vertex_data[face[0], :]
            vertex_data_f += vertex.tolist()
            vertex = vertex_data[face[1], :]
            vertex_data_f += vertex.tolist()
            vertex = vertex_data[face[2], :]
            vertex_data_f += vertex.tolist()

            indices += [index, index + 1, index + 2]
            index += 3

        return bs.Shape(vertex_data_f, indices)
